...
eta=10
kappa=0.5
#p_max=10 # P_max will be iterated
p_km_max=10
T_m=10
...
# Values of P_max to iterate over
p_max_values = range(1, 11) # P_max from 1 to 10
sum_best_fitness_vs_p_max = {} # Dictionary to store sum of best fitness for each P_max
...
for p_max in p_max_values:
    ...
    for l in range(num_bs):
        ...
        for k in range(num_uav_irs):
            ...
            Wl_value = Wl
            previous_best_fitness_gen = float('inf')
            # Initialize previous_best_individual_gen with a default structure including 'fitness'
            previous_best_individual_gen = {'fitness': float('inf'), 'data': {}}
    ...
    # Calculate sum of best fitness values for current p_max
    sum_best_fitness_current_p_max = sum([assignment['best_individual']['fitness'] for assignment in best_assignments])
    sum_best_fitness_vs_p_max[p_max] = sum_best_fitness_current_p_max
    print(f"P_max: {p_max}, Sum of Best Fitness: {sum_best_fitness_current_p_max:.4f}")


# Plotting graph for Sum of Fitness vs P_max
p_max_list = list(sum_best_fitness_vs_p_max.keys())
sum_fitness_list = list(sum_best_fitness_vs_p_max.values())

plt.figure(figsize=(10, 6))
plt.plot(p_max_list, sum_fitness_list, marker='o', linestyle='-')
plt.xlabel('P_max Value')
plt.ylabel('Sum of Best Fitness Values')
plt.title('Sum of Best Fitness Values vs. P_max using Random Search (Best-So-Far)')
plt.grid(True)
plt.xticks(p_max_list)
plt.tight_layout()
plt.show()