...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    ...
    plt.plot(D_m_current_values, fitness_sums_GA, label = "GA-A")
    plt.xlabel('Data size (Dm)',size=20)
    plt.ylabel('Energy',size=22)
    plt.legend()
    plt.savefig("Energy vs Data size _GA_parallel.pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
