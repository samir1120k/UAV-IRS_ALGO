...
def E_km_com(f_km,T_km_com, eta):
...
def run_ga_for_eta(eta_value):
            ...
            for j in range(num_generation):
                ...
                for i in range(0, population_size, 2): # Loop through population with step of 2
                    ...
                    def compute_fitness(data):
                        ...
                        E_kml_com_value = E_km_com(f_km_value, T_km_com_value, eta_value) # Use eta_value here
    ...
    # Calculate sum of best fitness values in the final generation
    ...
    return sum_best_fitness_final_gen


# Values of eta to iterate over
eta_values_to_test = range(1, 11) # eta from 1 to 10
sum_fitness_values = []

for eta_val in eta_values_to_test:
    sum_fitness = run_ga_for_eta(eta_val)
    sum_fitness_values.append(sum_fitness)
    print(f"For eta = {eta_val}, Sum of Best Fitness = {sum_fitness:.4f}")

# Plotting graph for eta vs Sum of Best Fitness
plt.figure(figsize=(10, 6))
plt.plot(eta_values_to_test, sum_fitness_values, marker='o', linestyle='-')
plt.xlabel('eta Value')
plt.ylabel('Sum of Best Fitness Values')
plt.title('Impact of eta on Sum of Best Fitness')
plt.grid(True)
plt.xticks(eta_values_to_test)
plt.tight_layout()
plt.show()