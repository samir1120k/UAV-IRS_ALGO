...
min_sum_fitness_overall = float('inf') # Initialize overall minimum fitness sum
...
for p_max_value in range(1, 11): # Iterate p_max from 1 to 10
    ...
    # sum_fitness_current_p_max += best_ind['fitness'] # Sum fitness values # already summed inside loop not needed here
    sum_fitness_current_p_max = sum(assignment['best_individual']['fitness'] for assignment in best_assignments if assignment['best_individual']['fitness'] != float('inf')) # Calculate sum of best fitness from auction

    if sum_fitness_current_p_max < min_sum_fitness_overall: # Compare with overall minimum
        min_sum_fitness_overall = sum_fitness_current_p_max # Update overall minimum

    fitness_sums_p_max.append(min_sum_fitness_overall) # Append overall minimum, not current sum
...
plt.ylabel('Minimum Sum of Best Fitness Values (Auctioned Assignments)') # Updated Y-axis label
plt.title('Minimum Sum of Best Fitness Values vs P_max') # Updated title
plt.grid(True)
