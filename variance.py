# coding: utf-8
#update the previous algorithm to iterate over variance
import pandas as pd
import numpy as np
import random
import math
...
num_irs_ele=50 # Fixed number of IRS elements
...
num_bs = 5
# num_irs_ele=50 # Fixed outside loop
num_uav_irs = 8
population_size = 50 # Population size for GA
all_best_combinations = []
all_best_individuals = []

# Values for variance to iterate over
variance_values = np.arange(0.1, 3.1, 0.1) # Variance values from 0.1 to 3.0 with step 0.1
fitness_results_vs_variance = {} # Store results for plotting
best_overall_fitness_variance = float('inf') # Initialize best overall fitness

for variance in variance_values:
    all_best_combinations_variance = [] # Store combinations for each variance value
    print(f"\n--- Starting GA for variance = {variance:.2f} ---")
        ...
        for k in range(num_uav_irs):
            ...
            for j in range(num_generation):
                ...
                for i in range(0, population_size, 2): # Loop through population with step of 2
                    ...
                    P_mutation = 0.5
                    # variance = 1 # Variance value is now from the outer loop
                    if u < P_mutation:
                    ...
                    def compute_fitness(data):
                        ...
                        if V_lm_hfly_value>0 and T_m_har_value>0 and T_ml_down_value>0 and T_km_up_value>0 and P_m_har_value<=p_max and P_m_down_value<=p_max and P_km_up_value<=p_km_max and (T_km_com_value+T_km_up_value+T_ml_down_value)<=T_m and f_km_value>0 and V_lm_vfly_value>0:
            ...
            all_best_combinations_variance.append({
                'bs_index': l,
                'uav_index': k,
    ...
    combination_lookup = {}
    for combination in all_best_combinations_variance: # Use combinations for current variance value
    ...
    # Calculate sum of best fitness values in the final generation for current variance
    sum_best_fitness_final_gen_variance = 0
    for assignment in best_assignments:
        sum_best_fitness_final_gen_variance += assignment['best_fitness'] # Use best_fitness directly which is from final generation

    # Compare with best_overall_fitness_variance and update if better
    if sum_best_fitness_final_gen_variance < best_overall_fitness_variance:
        best_overall_fitness_variance = sum_best_fitness_final_gen_variance
        fitness_results_vs_variance[variance] = best_overall_fitness_variance # Store the new best
    else:
        fitness_results_vs_variance[variance] = best_overall_fitness_variance # Store the previous best (or initial if no improvement)


    print(f"\n--- Completed GA for variance = {variance:.2f} ---")
    print(f"Sum of best fitness values in final generation (variance={variance:.2f}): {sum_best_fitness_final_gen_variance:.4f}")
    print(f"Current Best Overall Fitness (variance={variance:.2f}): {best_overall_fitness_variance:.4f}") # Print current best overall
    all_best_combinations.extend(all_best_combinations_variance) # Accumulate all combinations for potential further overall analysis if needed


# Plotting graph for variance vs Sum of Best Fitness
variance_plot = list(fitness_results_vs_variance.keys())
fitness_values_plot = list(fitness_results_vs_variance.values())

plt.figure(figsize=(10, 6))
plt.plot(variance_plot, fitness_values_plot, marker='o', linestyle='-')
plt.xlabel('Variance Value for Mutation')
plt.ylabel('Best Sum of Fitness Values (Maintained Best)')
plt.title('Impact of Mutation Variance on Best Sum of Fitness')
plt.grid(True)
plt.xticks(variance_plot)
plt.tight_layout()
plt.show()