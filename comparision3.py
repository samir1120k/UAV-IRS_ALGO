# coding: utf-8
#update the previous algorithm using Random Search with best-so-far strategy and compare GA, PSO, HC, and RS
...
# Fitness function to calculate total energy consumption (common for GA, PSO, HC and RS)
...
# Energy consumption of the UAV-IRS (common for GA, PSO, HC and RS)
...
# Power calculations for different flight modes (common for GA, PSO, HC and RS)
...
# Function to calculate energy and fitness (common for GA, PSO, HC and RS)
...
def run_hc(num_bs, num_uav_irs, num_generation, f_km, P_km_up, V_lm_vfly, V_lm_hfly):
    ...
    for l in range(num_bs):
        ...
        for k in range(num_uav_irs):
            ...
            for j in range(num_generation):
                ...
                current_fitness_hc, current_energy_data_hc = calculate_energy_and_fitness(current_individual_hc, l, k, Sub_value, 'HC')


                if best_fitness_hc == float('inf') or current_fitness_hc < best_fitness_hc: #For first iteration and better fitness
                    best_fitness_hc = current_fitness_hc
                    best_individual_hc = current_energy_data_hc.copy()


                generations_data_hc.append({'fitness': current_fitness_hc, 'data': current_energy_data_hc})

                # Generate neighbor solution (mutation in GA analogy)
                f_km_value_neighbor = f_km_value + random.uniform(-0.5, 0.5) #mutation step for f_km
                P_km_up_value_neighbor = P_km_up_value + random.uniform(-0.5, 0.5) #mutation step for p_km_up
                ...
                neighbor_fitness_hc, neighbor_energy_data_hc = calculate_energy_and_fitness(neighbor_individual_hc, l, k, Sub_value, 'HC')


                # Decide whether to move to neighbor
                if neighbor_fitness_hc < current_fitness_hc:
                    f_km_value = f_km_value_neighbor
                    P_km_up_value = P_km_up_value_neighbor


            best_individual_pair_hc = {'fitness': best_fitness_hc, 'data': best_individual_hc}
            best_individual_pair_hc['generation'] = j + 1 # Use last generation number for HC iterations
            best_individual_pair_hc['type'] = 'HC'
            best_individual_pair_hc['bs_index'] = l
            best_individual_pair_hc['uav_index'] = k
            all_best_individuals_bs_hc.append(best_individual_pair_hc)

            all_best_combinations_hc.append({
                'bs_index': l,
                'uav_index': k,
                'best_fitness': best_fitness_hc,
                'best_individual': best_individual_pair_hc,
                'generation_fitness': [gen['fitness'] for gen in generations_data_hc]
            })
            # print(f"Best Fitness for BS {l}, UAV {k}: {best_fitness_hc:.4f}")

        # Find best individual for current BS across all UAVs
        best_individual_for_bs_hc = min(all_best_individuals_bs_hc, key=lambda x: x['fitness'])
        # print(f"Best Fitness for BS {l} across all UAVs: {best_individual_for_bs_hc['fitness']:.4f}")

    return all_best_combinations_hc


def run_rs(num_bs, num_uav_irs, num_generation, population_size, f_km, P_km_up, V_lm_vfly, V_lm_hfly):
    all_best_combinations_rs = []
    all_best_individuals_rs = []

    for l in range(num_bs):
        all_best_individuals_bs_rs = []
        P_m_har_value = P_m_har.values[l]
        T_m_har_value = T_m_har.values[l]
        P_m_down_value = P_m_down.values[l]
        H_value = H
        ...
        for k in range(num_uav_irs):
            ...
            best_individual_rs = {}
            generations_data_rs = [] # To store best fitness of each generation for plotting
            V_lm_vfly_value = V_lm_vfly.values[k]
            V_lm_hfly_value = V_lm_hfly.values[k]
            D_l_hfly_value = D_l_hfly
            Wl_value = Wl
            ...
            for j in range(num_generation): # Generations for Random Search - iterations
                ...
                for i in range(population_size): # Samples per generation
                    ...
                    # Calculate fitness
                    result_fitness_rs, current_energy_data_rs = calculate_energy_and_fitness(current_individual_rs, l, k, Sub_value, 'RS_Best_So_Far')


                    if result_fitness_rs < best_fitness_gen:
                        best_fitness_gen = result_fitness_rs
                        best_individual_gen = current_energy_data_rs
                ...
                if best_fitness_gen < previous_best_fitness_gen:
                    ...
                    generations_data_rs.append({'fitness': best_fitness_gen, 'data': best_individual_gen.copy()}) # Append NEW best, now with fitness
                else:
                    # Keep the previous best and append it again for plotting
                    if j > 0: # For iterations after the first one
                        generations_data_rs.append({'fitness': previous_best_fitness_gen, 'data': previous_best_individual_gen.copy()}) # Append PREVIOUS best, now with fitness
                    else: # For the very first iteration, if no improvement, append the first best we found in this iteration
                        generations_data_rs.append({'fitness': best_fitness_gen, 'data': best_individual_gen.copy()}) # Append first best, now with fitness


                if previous_best_fitness_gen < best_fitness_rs: # Keep track of overall best
                    best_fitness_rs = previous_best_fitness_gen
                    best_individual_rs = previous_best_individual_gen


            best_individual_pair_rs = {'fitness': best_fitness_rs, 'data': best_individual_rs}
            best_individual_pair_rs['generation'] = j + 1 # Generation number (iterations)
            best_individual_pair_rs['type'] = 'RS_Best_So_Far' # Updated type to reflect strategy
            best_individual_pair_rs['bs_index'] = l
            best_individual_pair_rs['uav_index'] = k
            all_best_individuals_bs_rs.append(best_individual_pair_rs)


            all_best_combinations_rs.append({
                'bs_index': l,
                'uav_index': k,
                'best_fitness': best_fitness_rs,
                'best_individual': best_individual_pair_rs,
                'generation_fitness': [gen['fitness'] for gen in generations_data_rs] # Store fitness history
            })
            # print(f"Best Fitness for BS {l}, UAV {k}: {best_fitness_rs:.4f}")

        # Find best individual for current BS across all UAVs
        best_individual_for_bs_rs = min(all_best_individuals_bs_rs, key=lambda x: x['fitness'])
        # print(f"Best Fitness for BS {l} across all UAVs: {best_individual_for_bs_rs['fitness']:.4f}")

    return all_best_combinations_rs
...
num_generation_hc = 30 # Number of iterations for HC
num_generation_rs = 30 # Number of iterations for RS
num_uav_irs = 8
population_size_ga = 50  # Population size for GA
population_size_pso = 50  # Swarm size for PSO
population_size_rs = 50  # Samples per generation for RS
...
all_best_combinations_hc = run_hc(num_bs, num_uav_irs, num_generation_hc, f_km, P_km_up, V_lm_vfly, V_lm_hfly)
best_assignments_hc = select_best_assignments(all_best_combinations_hc, num_bs, num_uav_irs)

all_best_combinations_rs = run_rs(num_bs, num_uav_irs, num_generation_rs, population_size_rs, f_km, P_km_up, V_lm_vfly, V_lm_hfly)
best_assignments_rs = select_best_assignments(all_best_combinations_rs, num_bs, num_uav_irs)
...
algorithms = {'GA': {'best_assignments': best_assignments_ga, 'num_generation': num_generation_ga, 'all_best_combinations': all_best_combinations_ga},
              ...
              'HC': {'best_assignments': best_assignments_hc, 'num_generation': num_generation_hc, 'all_best_combinations': all_best_combinations_hc},
              'RS_Best_So_Far': {'best_assignments': best_assignments_rs, 'num_generation': num_generation_rs, 'all_best_combinations': all_best_combinations_rs}}

best_pair_for_plot_comparison = {}
min_fitness_for_plot_comparison = {'GA': float('inf'), 'PSO': float('inf'), 'HC': float('inf'), 'RS_Best_So_Far': float('inf')}
...
plt.title('Fitness Improvement Over Generations/Iterations for Best BS-UAV Pair (GA vs PSO vs HC vs RS)')
plt.grid(True)
plt.legend()
plt.xticks(range(0, max(num_generation_ga, num_generation_pso, num_generation_hc, num_generation_rs) + 2, 5))
plt.tight_layout()
plt.show()
...
if min_fitness_for_plot_comparison['GA'] != float('inf') and min_fitness_for_plot_comparison['PSO'] != float('inf') and min_fitness_for_plot_comparison['HC'] != float('inf') and min_fitness_for_plot_comparison['RS_Best_So_Far'] != float('inf'):
    best_algorithm = min(min_fitness_for_plot_comparison, key=min_fitness_for_plot_comparison.get)
    best_fitness_value = min_fitness_for_plot_comparison[best_algorithm]
    ...
    print(f"Best Fitness with HC: {min_fitness_for_plot_comparison['HC']:.4f}")
    print(f"Best Fitness with RS_Best_So_Far: {min_fitness_for_plot_comparison['RS_Best_So_Far']:.4f}")
    print(f"\n{best_algorithm} performed best with a fitness of {best_fitness_value:.4f}.")

    for algorithm_name, fitness_value in min_fitness_for_plot_comparison.items():
        if algorithm_name != best_algorithm:
            improvement = ((fitness_value - best_fitness_value) / fitness_value) * 100
            print(f"{best_algorithm} performed better than {algorithm_name} by {improvement:.2f}% relative to {algorithm_name}.")

else:
    print("\nCould not compare performance as best fitness was not found for all algorithms.")
...
plt.title('Sum of Best Fitness Values Across Generations/Iterations (GA vs PSO vs HC vs RS)')
