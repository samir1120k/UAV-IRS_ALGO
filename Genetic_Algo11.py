#updated the previous algorithm with condition check for computation time vs harvesting time
import pandas as pd
import numpy as np
import random
import math
...
for l in range(num_bs):
    ...
    for k in range(num_uav_irs):
        ...
        for i in range(50):
            ...
            T_km_com_value = D_km / f_km_value # Computation time calculation moved here to check the condition
            ...
            # Condition check: Ensure computation time is less than harvesting time
            if T_km_com_value >= T_m_har_value: # Condition check added here
                # Store current data if condition is met
            ...
            # else: implicitly discard the individual if condition is not met, by not appending to population
            ...
            for i in range(0, len(population), 2): # Corrected loop to pair population members
                ...
                def compute_fitness(data):  # Takes a dictionary of parameters as input
                    ...
                    if T_km_com_value >= T_m_har_value: # Condition check added here
                        ...
                        return float('inf'), None # Assign very high fitness if condition is not met, and no data

                child_fitness, child_data1 = compute_fitness(child_data)  # Compute child fitness

                if child_data1 is not None: # Only add valid child data (condition met)
                    child_population.append({
                        'fitness': child_fitness,
                        'data': child_data1
                    })
                # else: implicitly discard the child if condition is not met
