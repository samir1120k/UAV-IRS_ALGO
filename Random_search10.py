...
for l in range(num_bs):
    ...
    for k in range(num_uav_irs):
        ...
        for i in range(num_generation):
            f_km_value = f_km[random.randint(0,50)]
            P_km_up_value = P_km_up.values[random.randint(0,50)]

            Angle_row = Angle_df.iloc[random.randint(0,50), :] # Get row as Series
            h_l_m_row = h_l_m_df.iloc[random.randint(0,50), :] # Get row as Series
            h_l_km_row = h_l_km_df.iloc[random.randint(0,50), :] # Get row as Series
            Angle1_row = Angle_UP_df.iloc[random.randint(0,50), :] # Get row as Series
            g_l_m_row = g_l_m_df.iloc[random.randint(0,50), :] # Get row as Series
            g_l_km_row = g_l_km_df.iloc[random.randint(0,50), :] # Get row as Series
        ...
        best_individual_pair['generation'] = i + 1 # Use last j from loop, corrected index
        ...
        all_best_combinations.append({
            ...
            'generation_fitness': [gen['fitness'] for gen in population]
        })
