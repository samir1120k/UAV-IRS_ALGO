...
sigma_km=10**(-13)
num_population=50
num_generation = 30 # Number of generations # Fixed generation number
...
num_irs_ele_values = range(10, 51, 5) # Iterate from 10 to 50 with step 10
...
for num_irs_ele in num_irs_ele_values:
    ...
    fitness_results_vs_irs[num_irs_ele] = sum_best_fitness_final_gen
    print(f"\n--- Completed GA for num_irs_ele = {num_irs_ele} ---")
    print(f"Sum of best fitness values in final generation: {sum_best_fitness_final_gen:.4f}")
...
plt.title('Impact of Number of IRS Elements on Sum of Best Fitness')
plt.grid(True)
