...
p_max=10
p_km_max=10
# T_m=10 # T_m will be iterated
T_m_values = range(1, 11) # T_m values from 1 to 10
...
all_T_m_fitness_data = [] # List to store fitness data for each T_m value
...
for T_m in T_m_values: # Loop through T_m values
    ...
    # Calculate sum of fitness values for current T_m
    sum_fitness_T_m = sum([assignment['best_fitness'] for assignment in best_assignments])
    all_T_m_fitness_data.append({'T_m': T_m, 'sum_fitness': sum_fitness_T_m})

    print(f"\n--- Best Unique UAV Assignments (Auction Based Method) for T_m = {T_m} ---")
    ...
    # Plotting graphs (only for the last T_m value to avoid multiple plots during iteration, can be moved outside loop for all T_m plots if needed)
    if T_m == T_m_values[-1] and best_pair_for_plot: # Plot only for last T_m to avoid clutter, remove condition to plot for all T_m
        ...
        plt.title(f'Fitness Improvement Over Generations for Best BS-UAV Pair (BS {best_pair_for_plot['bs_index']}, UAV {best_pair_for_plot['uav_index']}) for T_m = {T_m}')
        plt.grid(True)
        plt.xticks(generations)
        plt.tight_layout()
        plt.show()
    elif T_m == T_m_values[-1]:
        print("\nNo best pair found for plotting.")


# Plotting Sum of Fitness vs T_m
T_m_plot_values = [item['T_m'] for item in all_T_m_fitness_data]
sum_fitness_plot_values = [item['sum_fitness'] for item in all_T_m_fitness_data]

plt.figure(figsize=(10, 6))
plt.plot(T_m_plot_values, sum_fitness_plot_values, marker='o', linestyle='-')
