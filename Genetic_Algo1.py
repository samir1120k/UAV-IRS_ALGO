...
results = []  # List to store fitness values
parent1 = None
parent2 = None
for i in range(2):
    H_value = random.choice(H.values)  # .values to get NumPy array element
    ...
    Bh = max(1, Bh) # Prevent negative Bh
    ...
    result = Fitness(E_ml_har_value, E_ml_down_value, E_ml_UAV_value)

    if parent1 is None:  # First iteration
        parent1 = result
    elif parent2 is None:  # Second iteration
        parent2 = result
    else:  # Subsequent iterations (if you have more than 2)
        if result > parent1:  # Example comparison (you can change this)
            parent2 = parent1
            parent1 = result
        elif result > parent2:
            parent2 = result
...
# print(results)  # Print the list of fitness values
