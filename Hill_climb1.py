    ...
    for k in range(num_uav_irs):
        ...
        for j in range(num_iterations_hc):
            ...
            def calculate_fitness_hc(individual_data):
                ...
                E_ml_har_value = P_m_har_value_hc * T_m_har_value_hc
...
# Print and Plotting for Hill Climbing
print("\n--- Best Unique UAV Assignments (Auction Based Method) - Hill Climbing ---")
best_pair_for_plot_hc = None
min_fitness_for_plot_hc = float('inf')

for assignment in best_assignments_hc:
    print(f"\nBest Assignment for BS {assignment['bs_index']} (Hill Climb):")
    print(f"  UAV Index: {assignment['uav_index']}")
    best_ind = assignment['best_individual']
    print(f"  Best Individual:")
    print(f"   Iteration: {best_ind['generation']}, Type: {best_ind['type']}") # Iteration instead of generation
    print(f"   Fitness: {best_ind['fitness']:.4f}") # Print current best fitness only
    for key, value in best_ind['data'].items():
        print(f"    {key}: {value:.4f}")
    print("-" * 20)

    if assignment['best_individual']['fitness'] < min_fitness_for_plot_hc:
        min_fitness_for_plot_hc = assignment['best_individual']['fitness']
        best_pair_for_plot_hc = assignment

# Unassigned BS/UAVs
if unassigned_bs_hc:
    print("\n--- Base Stations without Assigned UAVs (Hill Climb)---")
    for bs_index in unassigned_bs_hc:
        print(f"  BS {bs_index} : No UAV is assigned")
        print("-" * 20)

if unassigned_uavs_hc:
    print("\n--- UAVs without Assigned Base Stations (Hill Climb)---")
    for uav_index in unassigned_uavs_hc:
        print(f"  UAV {uav_index} : No BS is assigned")
        print("-" * 20)

# Plotting graphs for Hill Climbing
if best_pair_for_plot_hc:
    fitness_history_best_pair_hc = best_pair_for_plot_hc['generation_fitness'] # generation_fitness now stores fitness per iteration
    iterations = range(1, num_iterations_hc + 1) # Iterations for x-axis

    plt.figure(figsize=(10, 6))
    plt.plot(iterations, fitness_history_best_pair_hc, marker='o', linestyle='-') # Plot against iterations
    plt.xlabel('Iteration') # X-axis label as Iteration
    plt.ylabel('Fitness Value')
    plt.title(f'Fitness Improvement Over Iterations for Best BS-UAV Pair (BS {best_pair_for_plot_hc['bs_index']}, UAV {best_pair_for_plot_hc['uav_index']}) - Hill Climbing') # Updated title
    plt.grid(True)
    plt.xticks(iterations) # X-ticks as iterations
    plt.tight_layout()
    plt.show()
else:
    print("\nNo best pair found for plotting (Hill Climbing).")

# Sum of Best Fitness Values Across Iterations (Hill Climbing - Not really meaningful in HC context but kept for comparison)
sum_fitness_per_iteration_hc = [0] * num_iterations_hc # Initialize for iterations
for iter_idx in range(num_iterations_hc):
    iteration_sum = 0
    for combination in all_best_combinations_hc:
        iteration_sum += combination['generation_fitness'][iter_idx] # Use generation_fitness which is now iteration fitness
    sum_fitness_per_iteration_hc[iter_idx] = iteration_sum

iteration_indices_hc = list(range(1, num_iterations_hc + 1)) # Iterations for x-axis

plt.figure(figsize=(10, 6))
plt.plot(iteration_indices_hc, sum_fitness_per_iteration_hc, marker='o', linestyle='-') # Plot against iterations
plt.title('Sum of Best Fitness Values Across Iterations - Hill Climbing (Less Meaningful)') # Updated title
plt.xlabel('Iteration Number') # X-axis label as Iteration
plt.ylabel('Sum of Best Fitness Values')
plt.grid(True)
plt.xticks(iteration_indices_hc) # X-ticks as iterations
plt.show()