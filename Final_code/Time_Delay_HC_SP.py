...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    ...
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        fitness_sums_HC = pool.map(process_p_max_value, T_m_values)
    ...
    plt.plot(T_m_values, fitness_sums_HC, label = "HC-A")
    plt.xlabel('Time Delay (T_m)',size=20)
    plt.ylabel('Energy',size=22)
    plt.legend()
    plt.savefig("Energy vs Time Delay HC_parallel.pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
