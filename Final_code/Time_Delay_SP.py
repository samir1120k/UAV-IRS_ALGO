...
def process_p_max_value1(T_m):
    print(f"calculaiton for T_m for HC",T_m)
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    ...
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        ...
        fitness_sums_HC = pool.map(process_p_max_value1, T_m_values)
    ...
    plt.plot(T_m_values, fitness_sums_GA, label = "GA-A",marker='o', linestyle='-')
    plt.plot(T_m_values, fitness_sums_HC, label = "HC-A",marker='s', linestyle='dotted')
    plt.xlabel('Time Delay(T_m)',size=20)
    plt.ylabel('Energy',size=22)
    plt.legend()
    plt.savefig("Energy vs Time Delay(parallel)fix_PP.pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
    ...
    print(f"  HC-A was better in {better_algorithm_counts['HC-A']} out of {len(T_m_values)} cases.")
    print(f"  C2GA was better in {better_algorithm_counts['C2GA']} out of {len(T_m_values)} cases.")
    print(f"  Tie in {better_algorithm_counts['Tie']} out of {len(T_m_values)} cases.")
