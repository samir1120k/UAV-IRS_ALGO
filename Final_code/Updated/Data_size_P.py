...
def hc(hc_queue):
    ...
    Dm = np.arange(0.1, 1.1, 0.1) # P_max values from 1 to 11

    for D_m_current in Dm: # Iterate over P_max values
        ...
        if unassigned_uavs:
            ...
            for uav_index in unassigned_uavs:
                ...
                print("-" * 20)
    hc_queue.put(fitness_sums_HC) # Put fitness_sums_HC in the queue for HC 
...
if __name__ == "__main__":
    ...
    Dm = np.arange(0.1, 1.1, 0.1)

    plt.rcParams["font.size"] = "20"
    plt.plot(Dm, fitness_sums_HC, label = "HC-A")
    plt.plot(Dm, fitness_sums_GA, label = "C2GA")
    plt.xlabel('Data size',size=20)
    plt.ylabel('Energy',size=22)
    plt.legend()
    plt.savefig("Energy vs Data size(Gen=10).pdf", format="pdf", bbox_inches="tight", dpi=800)
    plt.show()
    ...
    print(f"  HC-A was better in {better_algorithm_counts['HC-A']} out of {len(Dm)} cases.")
    print(f"  C2GA was better in {better_algorithm_counts['C2GA']} out of {len(Dm)} cases.")
    print(f"  Tie in {better_algorithm_counts['Tie']} out of {len(Dm)} cases.")
