# coding: utf-8
#update the previous algorithm with Hill Climbing and compare GA, PSO and HC
...
f_km=np.random.uniform(0.1,1, 50) #Frequency of local computing
...
# Fitness function to calculate total energy consumption (common for GA, PSO and HC)
...
# Energy consumption of the UAV-IRS (common for GA, PSO and HC)
...
# Power calculations for different flight modes (common for GA, PSO and HC)
...
# Function to calculate energy and fitness (common for GA, PSO and HC)
                ...
                for i in range(0, population_size, 2): # Loop through population with step of 2
                    ...
                    if u < P_mutation:
                        for key in child_data_ga:
                            child_data_ga[key] += random.normal(loc=0, scale=2, size=(1))[0]
...
min_fitness_for_plot_comparison = {'GA': float('inf'), 'PSO': float('inf'), 'HC': float('inf')}
...
plt.title('Fitness Improvement Over Generations/Iterations for Best BS-UAV Pair (GA vs PSO vs HC)')
plt.grid(True)
plt.legend()
plt.xticks(range(0, max(num_generation_ga, num_generation_pso, num_generation_hc) + 2, 5))
plt.tight_layout()
plt.show()
...
if min_fitness_for_plot_comparison['GA'] != float('inf') and min_fitness_for_plot_comparison['PSO'] != float('inf') and min_fitness_for_plot_comparison['HC'] != float('inf'):
...
plt.title('Sum of Best Fitness Values Across Generations/Iterations (GA vs PSO vs HC)')
