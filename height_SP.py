...
def process_height_value(H_value): # Define function to process each height value
...
def process_height_value1(H_value): # Define function to process each height value
    print(f"calculaiton for H_value for HC",H_value)
    all_best_combinations = []
    all_best_individuals = []
    Bh = (1 - 2.2558 * pow(10, -5) *H_value)**4.2577
    p_l_b = (delta / 8) * Bh * Ar * s * pow(V_tip, 3)
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    height_values = range(10, 51, 5) #  values from 1 to 11
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        fitness_sums_GA = pool.map(process_height_value, height_values)
        fitness_sums_HC = pool.map(process_height_value1, height_values)
    ...
    plt.plot(height, fitness_sums_HC, label = "HC-A")
    plt.xlabel('height',size=20)
    plt.ylabel('Energy',size=22)
    plt.legend()
    plt.savefig("Energy vs height_HC(Gen=1)_parallel.pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
    print("Fitness sums HC:", fitness_sums_HC) # return this variable

# if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
#     with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
#         fitness_sums_GA = pool.map(process_height_value, Height_value)
...
#     plt.ylabel('Energy',size=22)
#     plt.legend()
#     # plt.savefig("Energy vs Height_GA(Gen=1)_parallel.pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
#     plt.show()
