...
min_rows = min(len(Angle_df), len(h_l_km_df), len(h_l_m_df), len(Angle_UP_df), len(g_l_km_df), len(g_l_m_df), len(Angle_har_df), len(f_l_km_df), len(f_l_m_df), len(f_km1), len(IRS_UP))
...
for D_m_current in D_m_values: # Iterate over D_km values
    ...
    for l in range(num_bs):
        ...
        Angle_df_bs = Angle_df.iloc[unique_row_indices, :].reset_index(drop=True)
        h_l_km_df_bs = h_l_km_df.iloc[unique_row_indices, :].reset_index(drop=True)
        h_l_m_df_bs = h_l_m_df.iloc[unique_row_indices, :].reset_index(drop=True)
        Angle_UP_df_bs = Angle_UP_df.iloc[unique_row_indices, :].reset_index(drop=True)
        g_l_km_df_bs = g_l_km_df.iloc[unique_row_indices, :].reset_index(drop=True)
        g_l_m_df_bs = g_l_m_df.iloc[unique_row_indices, :].reset_index(drop=True)
        Angle_har_df_bs = Angle_har_df.iloc[unique_row_indices, :].reset_index(drop=True)
        f_l_km_df_bs = f_l_km_df.iloc[unique_row_indices, :].reset_index(drop=True)
        f_l_m_df_bs = f_l_m_df.iloc[unique_row_indices, :].reset_index(drop=True)
        ...
        # print(f"Length of P_km_up: {len(P_km_up)}")
        # print(f"Maximum possible index for P_km_up: {len(P_km_up) - 1 if len(P_km_up) > 0 else -1}")
        # print(f"num_rows_data_files: {num_rows_data_files}")
        # print(f"population_size: {population_size}")
        # print(f"Example indices from unique_row_indices: {unique_row_indices[:10]}")
        # print(f"Max index in unique_row_indices: {max(unique_row_indices) if unique_row_indices else 'empty'}")
        # print(f"Length of valid_indices: {len(valid_indices)}") # Added debug print
        # print(f"Length of P_km_up_bs: {len(P_km_up_bs)}") # Added debug print
        ...
        for k in range(num_uav_irs):
            ...
            for i in range(len(valid_indices)): # Using length of valid_indices
                h_il_up_value=h_kml_down(Angle_UP_df_bs.iloc[i, :],g_l_m_df_bs.iloc[i, :],g_l_km_df_bs.iloc[i, :]) # Pass Series, corrected index to i
            ...
            for i in range(len(valid_indices)): # Using length of valid_indices
                ...
                Angle_row = Angle_df_bs.iloc[i, :] # Use BS-specific Angle_df
                h_l_m_row = h_l_m_df_bs.iloc[i, :] # Use BS-specific h_l_m_df
                h_l_km_row = h_l_km_df_bs.iloc[i, :] # Use BS-specific h_l_km_df
                Angle1_row = Angle_UP_df_bs.iloc[i, :] # Use BS-specific Angle_UP_df
                g_l_m_row = g_l_m_df_bs.iloc[i, :] # Use BS-specific g_l_m_df
                g_l_km_row = g_l_km_df_bs.iloc[i, :] # Use BS-specific g_l_km_df
                Angle2_row = Angle_har_df_bs.iloc[i, :] # Use BS-specific Angle_har_df
                f_l_m_row = f_l_m_df_bs.iloc[i, :] # Use BS-specific f_l_m_df
                f_l_km_row = f_l_km_df_bs.iloc[i, :] # Use BS-specific f_l_km_df


                # Verification: Check if Angle_row and Angle1_row are the same
                # if not Angle_row.equals(Angle1_row):
                    # print(f"INFO: Angle_row and Angle1_row are DIFFERENT for i={i}, l={l}, k={k}") # Changed WARNING to INFO
            ...
            for j in range(num_generation):
                ...
                for i in range(0, len(valid_indices), 2): # Loop through population with step of 2
                    ...
                    for key in parent1['data']:
                        # # Debug print to check data types during crossover
                        # print(f"Type of parent1['data'][key] for key '{key}': {type(parent1['data'][key])}")
                        # print(f"Type of parent2['data'][key] for key '{key}': {type(parent2['data'][key])}")
                    ...
                    def compute_fitness(data):
                        ...
                        T_l_vfly_value = H_value / V_lm_vfly_value
                        T_l_hfly_value = D_l_hfly_value / V_lm_hfly_value # Corrected: D_l_hfly / V_lm_hfly
                        E_ml_har_value = kappa * P_m_har_value * T_m_har_value # Corrected: Added kappa factor
