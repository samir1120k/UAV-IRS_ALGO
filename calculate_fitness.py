...
for m in range(num_base_stations):
    for l in range(num_uav_irs):
        ...
        P_m_har_values = [random.uniform(0.1, 10) for _ in range(10)]
        T_m_har_values = [random.uniform(1, 10) for _ in range(10)]
        ...
        E_ml_har_values = [E_ml_har(P, T) for P, T in zip(P_m_har_values, T_m_har_values)]
        ...
        Angle1 = np.exp(np.random.uniform(0, np.pi, num_uav_irs))  # Shape: (10,)
        ...
        E_ml_down_values = [E_ml_down(random.uniform(0, 10), T) for T in T_ml_down_values]
