    ...
    for l in range(num_bs):
        ...
        for k in range(num_uav_irs):
            ...
            for i in range(num_population):
                h_il_up_value=h_kml_down(Angle_UP_df.iloc[i, :],g_l_m_df.iloc[k, :],g_l_km_df.iloc[i, :]) # Pass Series
            ...
            for i in range(population_size):
                ...
                h_l_m_row = h_l_m_df.iloc[k, :] # Get row as Series
                h_l_km_row = h_l_km_df.iloc[i, :] # Get row as Series
                Angle1_row = Angle_UP_df.iloc[i, :] # Get row as Series
                g_l_m_row = g_l_m_df.iloc[k, :] # Get row as Series
                g_l_km_row = g_l_km_df.iloc[i, :] # Get row as Series
                Angle2_row = Angle_har_df.iloc[i, :] # Get row as Series
                f_l_m_row = f_l_m_df.iloc[k, :] # Get row as Series
                f_l_km_row = f_l_km_df.iloc[i, :] # Get row as Series


                # Verification: Check if Angle_row and Angle1_row are the same
                if not Angle_row.equals(Angle1_row):
                    print(f"WARNING: Angle_row and Angle1_row are DIFFERENT for i={i}, l={l}, k={k}")
                ...
                population.append({
                    ...
                    'data':  {
                        ...
                        'g_l_m_row':g_l_m_row,
                        'g_l_km_row':g_l_km_row,
                        'Angle_row':Angle_row,
                        'h_l_m_row':h_l_m_row,
                        'h_l_km_row':h_l_km_row,
                        'Angle2_row': Angle2_row,
                        'f_l_m_row': f_l_m_row,
                        'f_l_km_row': f_l_km_row
                        }
                    })
            ...
            for j in range(num_generation):
                ...
                for i in range(0, population_size, 2): # Loop through population with step of 2
                    ...
                    def compute_fitness(data):
                        ...
                        h_l_m_row = data['h_l_m_row'] # Retrieve h_l_m row
                        h_l_km_row = data['h_l_km_row'] # Retrieve h_l_km row
                        Angle1_row = data['Angle1_row'] # Retrieve angle row from child data
                        g_l_m_row = data['g_l_m_row'] # Retrieve g_l_m row
                        g_l_km_row = data['g_l_km_row'] # Retrieve g_l_km_row
                        Angle2_row = data['Angle2_row'] # Retrieve angle row from child data
                        f_l_m_row = data['f_l_m_row'] # Retrieve f_l_m row
                        f_l_km_row = data['f_l_km_row'] # Retrieve f_l_km row
                        ...
                        f_l_m_row_compute = f_l_m_df.iloc[k, :]
