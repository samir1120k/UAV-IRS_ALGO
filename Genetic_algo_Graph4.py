...
plots_directory = "genetic_algorithm_plots1"
...
num_generation = 200  # Number of generations
num_uav_irs = 8
...
bestfit_all_pairs = {} # Dictionary to store bestfit list for each BS-UAV pair
...
for l in range(num_bs):
    ...
    for k in range(num_uav_irs):
        ...
        # Store bestfit list for each BS-UAV pair
        bestfit_all_pairs[(l, k)] = bestfit[:]
...
# Calculate sum of fitness values for each generation across all BS-UAV pairs
generational_fitness_sum = []
for gen_num in range(num_generation):
    generation_sum = 0
    for l in range(num_bs):
        for k in range(num_uav_irs):
            if (l, k) in bestfit_all_pairs: #check if pair exists in dictionary, to avoid key error
                generation_sum += bestfit_all_pairs[(l, k)][gen_num] # Summing fitness for each pair in the generation
    generational_fitness_sum.append(generation_sum)


# Plotting for each BS-UAV pair (individual plots - already present)
for (bs_index, uav_index), bestfit_values in bestfit_all_pairs.items():
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, num_generation + 1), bestfit_values, marker='o')
    plt.xlabel('Generation')
    plt.ylabel('Best Fitness')
    plt.title(f'Best Fitness vs. Generation for BS {bs_index}, UAV {uav_index}')
    plt.grid(True)
    plt.xticks(np.arange(1, num_generation + 1, 1))
    plt.tight_layout()

    plot_filename = os.path.join(plots_directory, f"bs_{bs_index}_uav_{uav_index}_plot.png")
    plt.savefig(plot_filename)
    plt.close()

# Plotting the sum of fitness values for each generation
plt.figure(figsize=(10, 6))
plt.plot(range(1, num_generation + 1), generational_fitness_sum, marker='o')
plt.xlabel('Generation')
plt.ylabel('Sum of Best Fitness Values (All BS-UAV Pairs)')
plt.title('Sum of Best Fitness vs. Generation (All BS-UAV Pairs)')
plt.grid(True)
plt.xticks(np.arange(1, num_generation + 1, 1))
plt.tight_layout()

plot_filename = os.path.join(plots_directory, "sum_of_fitness_all_pairs_plot.png")
plt.savefig(plot_filename)
plt.close()


# Now plot ONLY the best pair (overall best plot - already present)
