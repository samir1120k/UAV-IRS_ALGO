...
fitness_sums_p_max_hc = [] # Store sum of fitness values for each p_max
...
for p_max_value in range(1, 11): # Iterate p_max from 1 to 10
    ...
    fitness_sums_p_max_hc.append(sum_fitness_current_p_max) # Store sum of fitness for this p_max

    # plt.figure(figsize=(10, 6))
    # plt.plot(generation_indices, sum_fitness_per_generation, marker='o', linestyle='-')
    # plt.title('Sum of Valid Fitness Values Explored Across Iterations (Hill Climb Search)') # Modified title for HC
    # plt.xlabel('Iteration Number') # Modified x-label for HC
    # plt.ylabel('Sum of Fitness Values')
    # plt.grid(True)
...
fitness_sum_for_p_max = {} # Dictionary to store sum of fitness for each p_max

previous_sum_fitness = float('inf') # Initialize previous sum fitness

for p_max_value in p_max_values:
    print(f"\n--- Running for p_max = {p_max_value} ---")
    all_best_combinations = []
    all_best_individuals = []
        ...
        for k in range(num_uav_irs):
            ...
            for j in range(num_generation):
                ...
                def compute_fitness(data):
                    ...
                    T_l_hfly_value = D_l_hfly_value / V_lm_hfly_value # Corrected: D_l_hfly / V_lm_hfly
                    E_ml_har_value = P_m_har_value * T_m_har_value
                    Angle_row_compute = Angle_df.iloc[i, :]  # Use same angle row as parent
                    h_l_m_row_compute = h_l_m_df.iloc[i, :]
                    h_l_km_row_compute = h_l_km_df.iloc[i, :]

                    Angle1_row_compute = Angle_har_df.iloc[i, :]  # Use same angle row as parent
                    f_l_m_row_compute = f_l_m_df.iloc[i, :]
                    f_l_km_row_compute = f_l_km_df.iloc[i, :]
                    ...
                    if V_lm_hfly_value<0 or T_m_har_value<0 or T_ml_down_value<0 or T_km_up_value<0 or P_m_har_value>p_max_value or P_m_down_value>p_max_value or P_km_up_value>p_km_max or (T_km_com_value+T_km_up_value+T_ml_down_value)>T_m or f_km_value<0 or V_lm_vfly_value<0 or E_kml_har_value<(E_kml_up_value+E_kml_com_value):
                        return float('inf'),{}
                    else:
    ...
    current_sum_fitness = sum(sum_fitness_per_generation)

    if current_sum_fitness > previous_sum_fitness: # If current sum is worse than previous
        fitness_sum_for_p_max[p_max_value] = previous_sum_fitness # Retain previous sum
    else:
        fitness_sum_for_p_max[p_max_value] = current_sum_fitness # Use current sum
        previous_sum_fitness = current_sum_fitness # Update previous sum for next iteration


# Plotting graph for p_max vs Sum of Fitness
p_max_plot_values = list(fitness_sum_for_p_max.keys())
fitness_sum_plot_values = list(fitness_sum_for_p_max.values())

# plt.figure(figsize=(10, 6))
# plt.plot(p_max_plot_values, fitness_sum_plot_values, marker='o', linestyle='-')
# plt.xlabel('p_max Value')
# plt.ylabel('Sum of Fitness Values Across Iterations')
# plt.title('Sum of Fitness Values vs. p_max')
# plt.grid(True)
# plt.xticks(p_max_plot_values)
# plt.tight_layout()
# plt.show()
...
# plt.plot(p_max_plot_values, [fitness_results[p_max]['Original'] for p_max in p_max_plot_values], marker='o', linestyle='-', label='Original')
plt.plot(p_max_range, fitness_sums_p_max_hc,marker='*', linestyle='--', label='hill climb')
# plt.plot(p_max_plot_values, [fitness_results[p_max]['Random Search'] for p_max in p_max_plot_values], marker='s', linestyle='--', label='Random Search')
plt.plot(p_max_range, fitness_sums_p_max, marker='s', linestyle='--', label='Genetic Algorithm')
plt.plot(p_max_range, fitness_sum_plot_values, marker='o', linestyle='--', label='Random Search')
plt.xlabel('P_max Value')
plt.ylabel('Sum of Best Fitness Values (Auctioned Assignments)')
plt.title('Comparison of Algorithm Performance vs. P_max')
plt.grid(True)
plt.xticks(p_max_plot_values)
plt.legend()
plt.tight_layout()
